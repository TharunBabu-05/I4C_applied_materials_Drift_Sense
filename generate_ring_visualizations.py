#!/usr/bin/env python3
"""
Generate individual visualizations for all ring dataset pairs using enhanced inference
"""
...
from inference import load_grayscale, preprocess_robust, resize_image, localize
...
def main():
    ...
    results_dir = data_dir / "results_final"
    ...
    print(f"Found {len(pair_dirs)} pairs to visualize with enhanced inference")
    ...
    for pair_dir in pair_dirs:
        ...
        # Run inference with robust preprocessing
        try:
            predicted = localize(str(ref_path), str(search_path), 
                               verbose=False, use_robust=True)
            error = compute_error(predicted, ground_truth)
            
            # Create visualization
            output_path = viz_dir / f"{pair_name}_enhanced_visualization.png"
    ...
    print(f"\nAll enhanced visualizations saved to: {viz_dir}")
