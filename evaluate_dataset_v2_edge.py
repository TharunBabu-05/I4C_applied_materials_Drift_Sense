#!/usr/bin/env python3
"""
Evaluate your inference on dataset_v2_two_noise_100_14-8-26 with edge enhancement mode
"""
...
def main():
    ...
    results_dir = Path(data_dir) / "results_yours_inference_edge"
    ...
    print("=" * 80)
    print("EVALUATING YOUR INFERENCE ON DATASET_V2_TWO_NOISE_100 (EDGE ENHANCEMENT MODE)")
    print("=" * 80)
    print(f"Data dir: {data_dir}")
    print(f"Pairs: {len(pair_dirs)}")
    print(f"Tolerance: 5 px")
    print(f"Mode: Edge Enhancement (--use_edge)")
    print("=" * 80)
    ...
    for pair_dir in pair_dirs:
        ...
        # Run inference with edge enhancement
        t_start = time.perf_counter()
        try:
            predicted = localize(str(ref_path), str(search_path), verbose=False, use_edge=True)
    ...
    # Print summary
    print("\n" + "=" * 80)
    print("EVALUATION SUMMARY (EDGE ENHANCEMENT MODE)")
    print("=" * 80)
