...
def generate_pair1():
    ...
    search_canvas = render_ic_layout(canvas_size=(th, tw), viewport=(0.0, 1000.0, 0.0, 1000.0), pair_id="pair1")
    
    # Apply ultra-heavy SEM noise to search image (sigma = 75.0)
    search_img_uint8 = apply_ultra_heavy_sem_noise(search_canvas, noise_sigma=75.0)

    # 2. Render 1000x1000 Target Reference Image (Zoomed In on [225, 225, 150, 150])
    target_canvas = render_ic_layout(canvas_size=(th, tw), viewport=(float(gt_x), float(gt_x + target_w), float(gt_y), float(gt_y + target_h)), pair_id="pair1")
    ...
    json_path = os.path.join(pair_dir, "pair1--groundtruth.json")
    ...
    gt_data = {
        ...
        "description": "Design 1: Dense Layout Top-Left Crossing Unique Contact Pattern (Noise sigma=75)"
    }

    with open(json_path, 'w') as f:
        json.dump(gt_data, f, indent=4)

    print(f"[Pair 1] Generated 1000x1000 search.png (noise sigma=75), 1000x1000 target.png, and pair1--groundtruth.json in {pair_dir}")
