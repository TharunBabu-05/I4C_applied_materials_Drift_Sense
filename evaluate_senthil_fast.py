#!/usr/bin/env python3
"""
Evaluate Senthil's fast inference on dense dataset
"""
...
import fast_inference_senthil as fast_inf
...
def main():
    data_dir = Path(r"C:\Semester-7\I4C_hackathon\dataset_most_denser_senthi")
    results_dir = data_dir / "results_senthil_fast"
    ...
    print("=" * 80)
    print("EVALUATING SENTHIL'S FAST INFERENCE ON DENSE DATASET")
    print("=" * 80)
    ...
    for pair_dir in pair_dirs:
        ...
        predicted = fast_inf.localize(str(ref_path), str(search_path), verbose=False)
    ...
    report = {
        "summary": {
            ...
            "tolerance_px": 5,
            "mean_error_px": round(float(errors_array.mean()), 2),
            "median_error_px": round(float(np.median(errors_array)), 2),
            "max_error_px": round(float(errors_array.max()), 2),
            "min_error_px": round(float(errors_array.min()), 2),
            "std_error_px": round(float(errors_array.std()), 2),
            "mean_time_ms": round(float(times_array.mean() * 1000), 2),
            "median_time_ms": round(float(np.median(times_array) * 1000), 2),
            "total_time_sec": round(float(times_array.sum()), 2)
        },
        "results": results
    }
